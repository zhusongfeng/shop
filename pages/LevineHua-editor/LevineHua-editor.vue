		...
		methods: {
			toolBarClick(type) {
				if (type == 'bold') {
					...
					if(this.myTextarea!=''){
						var provider={};
						provider.content = this.myTextarea.target.innerHTML;
						//存下本地的数据
			...
			submitImageLink(data) {
				...
				document.execCommand('insertHTML', false, "<image style='width:80%;margin-left:10%;' src='"+this.imageLink+"'></image>");
				this.$refs.qfAlert.imageLink = '';
			},
			isClean() {
				this.closeClean();
